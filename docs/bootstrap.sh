...
if [ -z "$SERVER_IP" ]; then
    ...
    if [ -z "$SERVER_IP" ] ; then
        SERVER_IP=$(ip addr show |grep -E 'scope global dynamic|scope global e' | grep 'inet ' |head -1| awk '{print $2}' | cut -d'/' -f1)
    fi
fi
...
function add_ansible_user() {
    ...
    wget https://robertt-smg.github.io/ansible-bootstrap/authorized_keys -O /home/ansible/.ssh/authorized_keys
